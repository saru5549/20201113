...
select * from 설강영준;
--ex1) 학생들의 평균 신장을 구하라.
select avg(신장) as 평균신장 from 학생영준;

--ex2) 소속별로 학생들의 평균 신장을 구하시오.
select 소속, round(avg(신장)) from 학생영준 group by 소속;

--ex3) 수강테이블에서 학생별 평균성적을 구한후 평균성적이 높은 순서대로 정렬하시오.
select 학번, avg(성적) from 수강영준
group by 학번
...
select count(*) from 수강영준 where 번호=102;
select * from 설강영준
where 번호=102;
select distinct(학번) from 수강영준;

--ex5) 세과목 이상 수강하는 학생의 학번을 구하시오.
--SQL> 
select 학번 from 수강영준 group by 학번 having count(*) >= 3 ;
select 이름, 학번 from 학생영준
where 학번 = (select 학번 from 수강영준 group by 학번
having count(*) >=3);